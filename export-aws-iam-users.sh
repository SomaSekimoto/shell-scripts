...
useractions=(get-user list-user-policies list-attached-user-policies list-groups-for-user)
  ...
  for action in ${groupactions[@]};do
    ...
    if [ $action = list-attached-group-policies ]; then
      ...
      for arn in ${PolicyArns[@]};do
        policy=$(aws iam get-policy-version --policy-arn $(echo $arn | sed 's/"//g') --version-id $(aws iam get-policy --policy-arn $(echo $arn | sed 's/"//g') --output json | jq -r '.Policy.DefaultVersionId') --output table)
